...
| `RestoreSnapshot` | namespaced | Operator-driven restore. Alternative to Mode A (annotation-on-pod) — either path is valid; users pick whichever fits their workflow. Targets an existing pod and drives CRIU replay against a referenced `Snapshot`. |
...
**Role.** The operator-driven restore object. Targets an existing pod and drives CRIU replay against a referenced `Snapshot`. An equal alternative to the annotation-driven restore path (section 4); users pick whichever fits their workflow. Provides a first-class CRD operators can `kubectl get`, watch, and report status against.
...
- Mode A (annotation-on-pod, section 4.1) and Mode B (this CRD) apply to disjoint pods — a pod is either restore-annotated or referenced from a RestoreSnapshot, not both.
...
Two equal alternatives. A given pod uses exactly one — modes apply to disjoint pods. Pick whichever fits the workflow.
